...
def test_s3_file_factory(s3_client: saritasa_s3_tools.S3Client) -> None:
    """Test S3FileField."""
    file_key = saritasa_s3_tools.factory.S3FileField(
        s3_config="files",
        s3_region="us-west-1",
        bucket="saritasa-s3-tools-files",
    ...
    s3_client.is_file_in_bucket(file_key), file_key


def test_s3_image_factory(s3_client: saritasa_s3_tools.S3Client) -> None:
    ...
    assert s3_client.is_file_in_bucket(file_key), file_key
